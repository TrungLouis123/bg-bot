...
const cooldownTime = 8 * 60 * 1000; // 24 hours in milliseconds
...
exports.run = async ({ api, event, Currencies }) => {
    ...
    if (!fs.existsSync(userDataPath)) {
        ...
        if (timeSinceLastDaily < cooldownTime) {
            ...
            return api.sendMessage(`${senderName} làm việc và nhận được ${randomMoney.toLocaleString()}$`, event.threadID);
        }
    }
};
